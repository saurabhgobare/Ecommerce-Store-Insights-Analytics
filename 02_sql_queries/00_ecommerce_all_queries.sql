...
-- FactSales view for Power BI sales analysis and dashboard reporting
Create view FactSales as
Select
    o.OrderID,o.OrderDate,o.IsReturned,

    c.CustomerID,c.CustomerName,c.CreatedAt as CustomerCreatedDate,

    r.RegionID,r.RegionName,r.Country,

    p.ProductID,p.ProductName,p.Category,p.Price as UnitPrice,

    od.Quantity,(od.Quantity * p.Price) as Revenue
From Orders o
join Customers c on o.CustomerID = c.CustomerID
join Regions r on c.RegionID = r.RegionID
join OrderDetails od on o.OrderID = od.OrderID
